...
###########################################    data detection         #######################################
    while True:
        ...
        im_data = images
        # im_info = np.multiply(edge_data["frame_shape"], edge_data["scale"]).tolist()
        # im_info.append(edge_data["scale"])
        ...
        for j in xrange(1, len(imdb)):  ######## nms ########
            ...
            if inds.numel() > 0:
                ...
                result, probs = bbox_detections(imdb[j], cls_dets.cpu().numpy(), 0.65)###############track need result#######
                if len(result) != 0:
