...
class resnet(_RFCN):
  def __init__(self, classes, num_layers=101, pretrained=False, pretrained_rfcn=False, class_agnostic=False):
    self.model_path = 'data/pretrained_model/resnet101_rcnn.pth'
    self.model_rfcn_path = "/home/wgq/RFCN-pytorch.1.0/rfcn_detect.pth" # trained on Imagenet VID+DET
  ...
  def _init_modules(self):
    ...
    #self.corr_bbox_net = nn.Conv2d(1051, 4*self.box_num_classes*7*7, [1,1], padding=0, stride=1)
  ...
  def _im_to_head(self, x):
    ...
    conv3 = self.RFCN_base[5](conv2)###############cut##################
    #print(self.RFCN_base[5])
