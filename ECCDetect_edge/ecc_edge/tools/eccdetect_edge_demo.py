...
def main():
    ...
    demo_out = cv2.VideoWriter('./demo/output/video.avi', fourcc, 25,(640,368))
        ...
        if ret == True:
            ...
            if first_frame:        
                ...
                for i in range(num_process):
                    cv2.rectangle(frame, (int(bbox[i][0]),int(bbox[i][1])),(int(bbox[i][2]),int(bbox[i][3])),(0, 0, 255), 3)
                    cv2.putText(frame, '%s: %.3f' % (label[i], probs[i]), (bbox[i][0], bbox[i][1] - 15), cv2.FONT_HERSHEY_PLAIN,1.0, (0, 0, 255), thickness=1)
                ...
                if is_key(key_frame, frame) or index % 10 ==0 :
                    ...
                    for i in range(num_process):
                        ...
                        cv2.putText(frame, '%s: %.3f' % (label[i], probs[i]), (bbox[0], bbox[1] - 15), cv2.FONT_HERSHEY_PLAIN, 1.0, (0, 255, 0), thickness=1)
                    index += 1
